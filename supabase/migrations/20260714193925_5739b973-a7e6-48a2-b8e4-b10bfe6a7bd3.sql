...
DROP FUNCTION IF EXISTS public.sp_convencion_vn_gerentes(int);
...
BEGIN
  ...
  vgm_dedup AS (
    SELECT DISTINCT gerente_n, celula_n, periodo, familia, unidades, acv FROM vgm_all
  ),
  ventas_by_gerente AS (
    ...
    FROM vgm_dedup GROUP BY gerente_n, periodo
  ),
  ...
  ventas_by_celula AS (
    SELECT celula_n, periodo,
           ...
           SUM(CASE WHEN familia='NUBE' THEN unidades ELSE 0 END) AS ventas_nube,
           SUM(acv) AS acv_total
    FROM vgm_dedup GROUP BY celula_n, periodo
  ),
  ventas_g AS (
    SELECT g.id AS gid,
