...
SELECT
  ...
  ROW_NUMBER() OVER (ORDER BY COALESCE(c.pct_cumplimiento, 0) DESC, t.acv_total DESC) AS posicion
FROM totales t
LEFT JOIN cumplimiento c ON t.comercial = c.comercial
ORDER BY pct_cumplimiento DESC, acv_total DESC;