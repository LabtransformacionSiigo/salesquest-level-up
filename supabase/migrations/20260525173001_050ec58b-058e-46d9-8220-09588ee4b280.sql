...
AS $$
BEGIN
  -- 1) Reconstruir meta_fe / meta_nube / meta_total_und por célula+mes
  --    desde metas_asesores. En MEX el residuo (meta_total - meta_fe) pertenece
  --    a la familia NUBE (Aspel COI/NOI/SAE, Gestión, ADM Premium).
  WITH agg AS (
    SELECT
      celula,
      anio_mes,
  ...
  -- 2) Backfill ACV por ratio (igual que antes)
  WITH ratios AS (
    SELECT canal,
...
-- Ejecutar una vez ahora para corregir el histórico
