...
BEGIN
  ...
  SELECT jsonb_build_object(
    'conSpTotal', (
      SELECT count(DISTINCT g.id) FROM public.gerentes g JOIN led ON led.gerente_id = g.id
      WHERE led.total > 0 AND NOT (g.pais='MEX' AND g.canal='VC')
    ),
    'conCanje', (
      SELECT count(DISTINCT g.id) FROM public.gerentes g JOIN led ON led.gerente_id = g.id
      WHERE led.canje > 0 AND NOT (g.pais='MEX' AND g.canal='VC')
    ...
    'spTotalGlobal', (
      SELECT COALESCE(SUM(led.total),0) FROM public.gerentes g JOIN led ON led.gerente_id = g.id
      WHERE NOT (g.pais='MEX' AND g.canal='VC')
    )
  ) INTO v_resumen_sp;
