...
SELECT d::date::text, 47 + row_number() OVER (ORDER BY d), false, NULL
FROM generate_series('2026-07-18'::date, CURRENT_DATE, interval '1 day') AS d
WHERE NOT EXISTS (
  SELECT 1 FROM public._backfill_vn_junio b WHERE b.fecha::date = d::date
);
