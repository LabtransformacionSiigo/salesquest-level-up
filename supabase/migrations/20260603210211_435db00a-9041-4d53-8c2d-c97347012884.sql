...
BEGIN
  -- Permitir cron (auth.uid() IS NULL) o admin/especialista
  ...
  WITH ganados AS (
    ...
    FROM sp_acumulados WHERE tipo_sp = 'canje' GROUP BY gerente_id
  ),
  gastados AS (
    SELECT gerente_id, SUM(puntos_gastados)::int AS gastado
    FROM canjes WHERE estado <> 'rechazado' GROUP BY gerente_id
  ),
  upd AS (
    UPDATE gerentes g
