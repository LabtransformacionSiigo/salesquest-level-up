...
CREATE OR REPLACE FUNCTION public.sp_convencion_vn_gerentes(anio int DEFAULT 2026)
...
BEGIN
  ...
  WITH
  norm AS (
    -- helper: normalize function inlined via lower(unaccent(regexp_replace(...)))
    SELECT 1
  ),
  gers AS (
  ...
  metas_ranked AS (
    SELECT celula_n, periodo, meta_fe, meta_nube, meta_acv,
           ROW_NUMBER() OVER (
             PARTITION BY celula_n, periodo
             ORDER BY is_cierre DESC
           ) AS rn
    FROM metas_raw
  ...
  vgm_all AS (
    ...
    WHERE v.periodo::text LIKE anio::text || '%'
  ),
  vgm_dedup AS (
    SELECT DISTINCT gerente_n, celula_n, periodo, familia, unidades, acv
    FROM vgm_all
  ),
  -- ventas por gerente_n
  ventas_by_gerente AS (
    ...
    FROM vgm_dedup
    GROUP BY gerente_n, periodo
  ),
  gerentes_con_ventas AS (
    SELECT DISTINCT gerente_n FROM vgm_dedup WHERE gerente_n <> ''
  ),
  ventas_by_celula AS (
    SELECT celula_n, periodo,
           ...
           SUM(acv) AS acv_total
    FROM vgm_dedup
    GROUP BY celula_n, periodo
  ),
  -- Para cada gerente elige fuente: por nombre si tiene, si no por celula
  ventas_g AS (
    SELECT g.id AS gid,
           ...
           CASE WHEN EXISTS (SELECT 1 FROM gerentes_con_ventas gv WHERE gv.gerente_n = g.nombre_n)
                THEN 'nombre' ELSE 'celula' END AS fuente
    FROM gers g
  ...
  ventas_final AS (
    SELECT gid, periodo, ventas_fe, ventas_nube, acv_total
    FROM ventas_g WHERE periodo IS NOT NULL
  ),
  metas_g AS (
    SELECT g.id AS gid, m.periodo, m.meta_fe, m.meta_nube, m.meta_acv
    FROM gers g
    JOIN metas m ON m.celula_n = g.celula_n
    WHERE m.periodo LIKE anio::text || '%'
  ),
  combined AS (
  ...
  calc AS (
    SELECT c.gid, c.periodo,
           ...
           COALESCE(m.meta_fe,0)     AS meta_fe,
           COALESCE(m.meta_nube,0)   AS meta_nube,
           COALESCE(m.meta_acv,0)    AS meta_acv
    FROM combined c
    LEFT JOIN ventas_final v ON v.gid=c.gid AND v.periodo=c.periodo
    LEFT JOIN metas_g m ON m.gid=c.gid AND m.periodo=c.periodo
  ),
  sp_mes AS (
    SELECT gid,
  ...
  FROM gers g
  LEFT JOIN totals t ON t.gid = g.id;
END;
$$;

REVOKE ALL ON FUNCTION public.sp_convencion_vn_gerentes(int) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.sp_convencion_vn_gerentes(int) TO service_role;

COMMENT ON FUNCTION public.sp_convencion_vn_gerentes(int) IS
'Solo lectura. Replica computeSpConvencionAnualForCelula (src/lib/sp-convencion-anual.ts): SP Convención anual de gerentes VN calculado en vivo desde metas_acv_gerentes (Cierre>Inicio) y ventas_gerente_mensual (match por nombre, fallback celula, dedup exacto). Fórmula por mes: cap(%FE) + cap(%NUBE)*2 + cap(%ACV), cap=min(300,max(0,round(x))). No modifica datos.';
