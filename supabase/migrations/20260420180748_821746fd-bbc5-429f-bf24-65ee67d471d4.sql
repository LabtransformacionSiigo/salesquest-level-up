...
-- Normalize canal: ejecucion uses 'Empresarios', metas_asesores uses 'SMBS'
metas_norm AS (
  SELECT 
    nombre_asesor,
    celula,
    novedad,
    meta_fe, meta_nube, meta_total,
    CASE WHEN canal_direccion = 'SMBS' THEN 'Empresarios' ELSE canal_direccion END AS canal_direccion,
    anio_mes
  FROM metas_asesores
  WHERE anio_mes = (SELECT periodo FROM mes_actual)
),
-- Aggregate execution per (celula, canal) joining by NAME
ejec_celula AS (
  SELECT 
    mn.celula,
    mn.canal_direccion,
  ...
  JOIN metas_norm mn 
    ON LOWER(TRIM(ea.documento_asesor)) = LOWER(TRIM(mn.nombre_asesor))
   AND ea.canal_direccion = mn.canal_direccion
  WHERE ea.periodo = (SELECT periodo FROM mes_actual)
    AND (mn.novedad IS NULL OR LOWER(TRIM(mn.novedad)) IN ('', 'sin novedad', 'no aplica'))
    AND mn.celula IS NOT NULL
  GROUP BY mn.celula, mn.canal_direccion
),
meta_celula AS (
  SELECT 
    celula,
...
LEFT JOIN ejec_celula ec 
  ON LOWER(TRIM(ec.celula)) = LOWER(TRIM(g.celula))
...
LEFT JOIN meta_celula mc 
  ON LOWER(TRIM(mc.celula)) = LOWER(TRIM(g.celula))
...
LEFT JOIN meta_acv_celula mac 
  ON LOWER(TRIM(mac.celula)) = LOWER(TRIM(g.celula))
