// Edge function: evaluar-retos-vn
// Evalúa retos diarios (Golazo del Día), semanales (Jugada de la Semana),
// mensuales (Bota de Oro), rachas y medallas para gerentes VN (COL/ECU).
...
const isNube = (sale: any): boolean => {
  const raw = `${sale.producto || ""} ${sale.categoria_producto_venta || ""} ${
    sale.bloque_venta || ""
  }`
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const anioMes = fechaStr.slice(0, 7);
    ...
    for (const gerente of gerentes) {
      ...
      const { data: asesoresData } = await supabase
        ...
        .eq("activo", true);
      const documentos = (asesoresData ?? [])
        .map((a: any) => a.documento)
        .filter(Boolean);
      ...
      const { data: kpiRow } = await supabase
        ...
        .eq("anio_mes", anioMes)
        .maybeSingle();
      const metaMensualACV = Number(kpiRow?.meta ?? 0);
      ...
      const { data: metaNubesRow } = await supabase
        ...
        .eq("anio_mes", anioMes)
        .maybeSingle();
      const metaMensualNubes = Number(metaNubesRow?.meta_nubes ?? 0);
      ...
      for (const reto of retos) {
        ...
        if (reto.tipo === "DIARIO") {
          ...
          const rachaConf = rachas.find(
            (r: any) => r.tipo === "DIARIA" && r.reto_ref_id === reto.id,
          );
          let spBase = cumple ? reto.sp_base : 0;
          let spConRacha = spBase;
          let rachaAct: any = null;
          ...
          if (rachaConf) {
            const { data: er } = await supabase
              ...
              .maybeSingle();
            if (cumple) {
              const cons = (er?.dias_o_semanas_consecutivas ?? 0) + 1;
              const yaActiva = er?.racha_activa === true;
              const activaAhora = cons > rachaConf.dias_consecutivos_requeridos;
              if (yaActiva || activaAhora) {
                spConRacha = Math.round(
                  spBase * Number(rachaConf.multiplicador),
                );
              }
              rachaAct = {
                racha_id: rachaConf.id,
                gerente_id: gerente.id,
                dias_o_semanas_consecutivas: cons,
          ...
          if (!dryRun) {
            await supabase.from("retos_vn_progreso_diario").insert({
              ...
              cumple,
              sp_otorgados: spBase,
              sp_con_racha: spConRacha,
            });
            if (rachaAct) {
              await supabase
                .from("rachas_vn_estado")
                .upsert(rachaAct, { onConflict: "racha_id,gerente_id" });
            }
          ...
          resultados.push({
            ...
            fecha: fechaStr,
            nubesVendidas,
            metaDiaria: Math.ceil(metaDiaria),
        ...
        if (reto.tipo === "SEMANAL") {
          ...
          const semana = semanas.find(
            (s) => fechaStr >= s.fecha_inicio && fechaStr <= s.fecha_fin,
          );
          if (!semana) continue;
          if (fechaStr !== semana.fecha_fin) {
            resultados.push({
              gerente: gerente.nombre,
              reto: reto.nombre,
              resultado: `Sem ${semana.numero} no terminada aún`,
            });
            continue;
          }
          ...
          const acvReal = (ventasSem ?? []).reduce(
            (s: number, v: any) => s + Number(v.acv_plus ?? 0),
            0,
          );
          ...
          let rachaActSem: any = null;

          const rachaConfSem = rachas.find(
            (r: any) => r.tipo === "SEMANAL" && r.reto_ref_id === reto.id,
          );
          if (rachaConfSem) {
            const { data: es } = await supabase
              ...
              .maybeSingle();
            if (cumpleSem) {
              const cons = (es?.dias_o_semanas_consecutivas ?? 0) + 1;
              const yaActiva = es?.racha_activa === true;
              const activaAhora =
                cons > rachaConfSem.dias_consecutivos_requeridos;
              if (yaActiva || activaAhora) {
                spConRachaSem = Math.round(
                  spBaseSem * Number(rachaConfSem.multiplicador),
                );
              }
              rachaActSem = {
                racha_id: rachaConfSem.id,
                gerente_id: gerente.id,
                dias_o_semanas_consecutivas: cons,
                racha_activa: activaAhora || yaActiva,
                ultima_fecha_cumplida: fechaStr,
          ...
          if (!dryRun) {
            await supabase.from("retos_vn_progreso_semanal").insert({
              ...
              fecha_inicio_semana: semana.fecha_inicio,
              fecha_fin_semana: semana.fecha_fin,
              acv_real: acvReal,
              meta_semanal_acv: metaSemanal,
              pct_cumplimiento: pctSem,
              cumple: cumpleSem,
              sp_otorgados: spBaseSem,
              sp_con_racha: spConRachaSem,
            });
            if (rachaActSem) {
              await supabase
                .from("rachas_vn_estado")
                .upsert(rachaActSem, { onConflict: "racha_id,gerente_id" });
            }
            if (cumpleSem && spConRachaSem > 0) {
              spInserts.push({
                ...
                fuente: "RETO_SEMANAL",
                sp: spConRachaSem,
                periodo: `${anioMes}-S${semana.numero}`,
                tipo_sp: "canje",
          ...
          resultados.push({
            ...
            acvReal: Math.round(acvReal),
            metaSemanal: Math.round(metaSemanal),
            pct: `${(pctSem * 100).toFixed(1)}%`,
            cumple: cumpleSem,
            spBase: spBaseSem,
            spConRacha: spConRachaSem,
            dry_run: dryRun,
          });
        }
        ...
        if (reto.tipo === "MENSUAL") {
          const ultimoDia = new Date(
            Date.UTC(
              fechaEval.getUTCFullYear(),
              fechaEval.getUTCMonth() + 1,
              0,
            ),
          )
            .toISOString()
          ...
          if (!dryRun) {
            await supabase.from("retos_vn_progreso_mensual").insert({
              ...
              anio_mes: anioMes,
              acv_real: acvMes,
              meta_mensual_acv: metaMensualACV,
              pct_cumplimiento: pctMes,
              cumple: cumpleMes,
              sp_otorgados: spMes,
            });
          ...
          resultados.push({
            ...
            tipo: "MENSUAL",
            mes: anioMes,
            acvMes: Math.round(acvMes),
            metaMensual: Math.round(metaMensualACV),
            pct: `${(pctMes * 100).toFixed(1)}%`,
            cumple: cumpleMes,
            sp: spMes,
            dry_run: dryRun,
          });
    ...
    if (!dryRun && spInserts.length > 0) {
      ...
      const gids = [...new Set(spInserts.map((s) => s.gerente_id))];
      for (const gid of gids) {
        const total = spInserts
          .filter((s) => s.gerente_id === gid)
          .reduce((s, x) => s + x.sp, 0);
        await supabase.rpc("increment_gerente_sp_canje", {
          p_gerente_id: gid,
          p_delta: total,
        });
      }
