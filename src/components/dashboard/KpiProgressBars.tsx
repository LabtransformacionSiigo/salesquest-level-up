...
const KpiProgressBars = ({ kpis, acvMes, ventasSemana, isVcAdvisor, loading, pctCumplimiento, sp = 0, canal, pais, ejecucion, metaAsesor, isVCGerente, teamAsesorPerformance, vcCumplimiento, periodoSeleccionado, lastUpdated }: KpiProgressBarsProps) => {
  ...
  return (
    <motion.div className="grid grid-cols-1 lg:grid-cols-2 gap-5" variants={fadeUpItem}>
      ...
      >
        <h3 className="text-base font-bold font-heading text-secondary mb-1 flex items-center gap-2">
          <span className="text-primary">🎯</span> {showVCTeam || showVNTeam ? `AVANCE DEL EQUIPO (${teamAsesorPerformance?.length || 0} ASESORES)` : 'AVANCE DEL MES'}
        </h3>
